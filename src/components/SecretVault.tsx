...
export default function SecretVault() {
    ...
    // Ambil nama dari Scanner, kalau kosong default ke 'GUEST'
    const agentName = location.state?.agentName || "UNKNOWN_AGENT";
    // Ubah spasi jadi garis bawah (underscore) dan huruf besar semua
    const formattedName = agentName.toUpperCase().split(' ').join('_');
  ...
  useEffect(() => {
    const updateClock = () => {
      const now = new Date();
      // Format as WITA time representation for effect (UTC+8 roughly)
  ...
  // Perhatikan ada 'onClick' di dalam { } dan tipe datanya di sebelah kanan
    const ActionButton = ({ icon: Icon, label, danger, onClick }: { icon: any, label: string, danger?: boolean, onClick?: () => void }) => (
    <button 
        onClick={onClick} // <--- SEKARANG INI SUDAH DEFINED KARENA ADA DI ATAS
        className={`flex-1 neon-border hover:bg-[var(--emerald)] hover:text-black transition-all text-[10px] font-bold tracking-widest flex items-center justify-center gap-2 py-3 lg:py-0 ${danger ? 'bg-red-950/20 border-red-900 text-red-500 hover:bg-red-500' : 'bg-[#00ff88]/10 text-[var(--emerald)]'}`}
    >
  ...
  return (
    <div className="h-screen w-full flex flex-col p-4 sm:p-6 gap-4 relative selection:bg-[var(--emerald)] selection:text-black">
      ...
      {/* Background Vignette and CRT Effects */}
      ...
      <motion.div 
            // --- MASUKKAN BUMBU ANIMASI DI SINI ---
        ...
        {/* Main Grid Layout from Immersive UI */}
            ...
            <div className="flex flex-col gap-3 flex-grow overflow-y-auto pr-2 custom-scrollbar">
              {personnel.map((p, index) => (
                <div key={index} className="flex justify-between items-center group cursor-crosshair">
                  <span className="group-hover:text-[var(--emerald)] transition-colors text-sm opacity-90">{p.name}</span>
                  <span className="text-emerald-700 group-hover:text-emerald-500 text-[10px] uppercase font-bold tracking-wider transition-colors">{p.role}</span>
                </div>
              ))}
            </div>
          </div>
            ...
            <div className="space-y-6 flex-grow flex flex-col justify-between">
              ...
              <div className="flex items-center justify-between p-3 border border-emerald-900 bg-emerald-950/20">
                <div className="flex flex-col">
                  ...
                  <span className="text-lg font-bold neon-text mt-1 leading-tight text-[var(--emerald)]">OPTIMAL</span>
                </div>
        ...
        <div className="lg:col-span-12 lg:row-span-2 flex flex-col neon-border rounded-sm p-4 bg-black/80 font-mono text-xs overflow-hidden">
          ...
          <div className="flex flex-col gap-1 overflow-y-hidden opacity-80 space-y-1 custom-scrollbar">
            {logs.map((log, i) => (
              <div key={i} className={`${i === logs.length - 1 ? 'animate-[pulse_2s_ease-in-out_infinite] neon-text' : 'opacity-80'}`}>
                {log}
              </div>
            ))}
        ...
        {/* Action Buttons */}
            ...
            {/* Panggil fungsi handleTerminate di sini */}
            <ActionButton 
                icon={LogOut} 
    ...
    </motion.div> {/* Tutup Wrapper Animasi */}
    </div> /* Tutup Root Div */
  );
}